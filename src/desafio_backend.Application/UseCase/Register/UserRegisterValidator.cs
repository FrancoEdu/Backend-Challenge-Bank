...
public class UserRegisterValidator : AbstractValidator<UserRegisterRequestJson>
{
    public UserRegisterValidator()
    {
        ...
        RuleFor(x => x.Email).NotEmpty()
            .WithMessage(ResourceErrorMessage.EMAIL_REQUIRED)
            .EmailAddress()
        ...
        RuleFor(x => x.Password).NotEmpty().WithMessage(ResourceErrorMessage.PWD_REQUIRED)
            ...
            .Equal(x => x.ConfirmPassword).WithMessage(ResourceErrorMessage.PASSWORD_NOT_EQUALS);
        RuleFor(x => x.CpnjCpf).NotEmpty().WithMessage(ResourceErrorMessage.CPF_REQUIRED);
    }
}
